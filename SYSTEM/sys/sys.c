...
/*
	函数名称：void SetNVICPriority(IRQn_Type IRQn,uint32_t PreemptPriority,uint32_t SubPriority)
	函数功能：设置中断线 抢占优先级和子优先级
	函数形参：
					IRQn_Type IRQn：中断线
					uint32_t PreemptPriority：抢占优先级(0 - 3)
					uint32_t SubPriority：子优先级(0 - 3)
	返回值：
*/
void SetNVICPriority(IRQn_Type IRQn,uint32_t PreemptPriority,uint32_t SubPriority)
{
	...
	//设置抢占优先级和子优先级，并且返回一个形参
	NVIC_SetPriority(IRQn,priority);//设置中断线
